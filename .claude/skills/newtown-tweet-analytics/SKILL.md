...
| ファイル                                                                         | 目的                                                     | 対象                             |
| -------------------------------------------------------------------------------- | -------------------------------------------------------- | -------------------------------- |
| `project/newtown-analytics/knowledge/patterns.md`                                | ツイート生成スキルが参照する構造化ナレッジ（上書き更新） | `newtown-multiview-tweet` スキル |
| `project/newtown-analytics/data/analytics_data/{YYYY-MM-DD}-{連番}-analytics.md` | ユーザー向けの全体分析レポート（毎回新規作成）           | 施策判断・仮説検証用             |

## 入力素材（任意）

$ARGUMENTS

入力がない場合は `project/newtown-analytics/data/tweet_analytics/` 内の全CSVを対象にします。
...
**入力**: `project/newtown-analytics/data/tweet_analytics/*.csv`
**出力**: `project/newtown-analytics/knowledge/analysis_output.json`
...
全tmpファイルは `project/newtown-analytics/tmp/` に保存される。

`newtown-analytics-ga` エージェントは `project/newtown-analytics/data/google-analytics/*.csv` を直接読み込む（`analysis_output.json` に依存しないため、Step 1と同時に起動しても構わないが、実装上は他の並列エージェントと同タイミングで起動する）。
...
**出力**: `project/newtown-analytics/tmp/analytics-hypothesis-[YYYYMMDD].md`
...
**出力**: `project/newtown-analytics/knowledge/patterns.md`（上書き）
...
**入力**: Step 2〜7の全tmpファイル（hook/hashtag/phase/algorithm/ga/hypothesis） + `analysis_output.json`
**出力**: `project/newtown-analytics/data/analytics_data/{YYYY-MM-DD}-{連番}-analytics.md`（新規）
...
- **ツイートCSV**: `project/newtown-analytics/data/tweet_analytics/*.csv`（Pythonスクリプトが自動で全件読み込む）
- **Google Analytics CSV**: `project/newtown-analytics/data/google-analytics/*.csv`（GAエージェントが直接読み込む）
- **Xアルゴリズム**: `knowledge/x_algorithm/`
- **中間出力**: `project/newtown-analytics/knowledge/analysis_output.json`
